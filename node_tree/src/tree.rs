...
use bevy::{ecs::{bundle::DynamicBundle, component::Components, storage::Storages}, prelude::*, utils::HashMap};
...
impl NodeTree {
    ...
    pub fn with<T: Component + Any>(mut self, component : T) -> Self {
        ...
        self.components.insert(TypeId::of::<T>(), ComponentHolder::Typed(TypedComponentHolder::new(component)));
    ...
    pub fn with_bundle<T: DynamicBundle + Bundle>(mut self, bundle : T) -> Self {
        ...
        for (idx, ptr) in owning_ptrs.into_iter().enumerate() {
            ...
            self.components.insert(type_id, ComponentHolder::Raw(holder));
        }
...
mod tests {
    ...
    fn test_with_component() {
        ...
        assert_eq!(entity.get::<TestComponent>().cloned(), Some(TestComponent(42)));
    }

    #[test]
    fn test_with_bundle() {
        let entity = NodeTree::default()
            .with_bundle(TestBundle {
                a: TestComponent(42),
        ...
        assert_eq!(entity.get::<TestComponent>().cloned(), Some(TestComponent(42)));
        assert_eq!(entity.get::<AnotherComponent>().cloned(), Some(AnotherComponent("Hello".to_string())));
    }

    #[test]
    fn test_with_child() {
        let child = NodeTree::default()
            .with(TestComponent(24));
        ...
        assert_eq!(entity.children[0].get::<TestComponent>().cloned(), Some(TestComponent(24)));
    }

    #[test]
    fn test_with_children() {
        let child1 = NodeTree::default()
            .with(TestComponent(24));

        let child2 = NodeTree::default()
            .with(AnotherComponent("World".to_string()));
        ...
        assert_eq!(entity.children[1].get::<AnotherComponent>().cloned(), Some(AnotherComponent("World".to_string())));
    }
}