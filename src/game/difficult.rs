...
pub const EVENT_LOOP_DURATION : f32 = EVENTS_IN_LOOP * (EVENT_DISTANCE + EVENT_DURATION) + WORKS_IN_LOOP; 
...
pub const FIRE_RATE: f32 = (RES_LIMIT * 0.6 - EVENT_LOOP_DURATION * BREATH_RATE + HYDROPONIC_UP_TIME * HYDROPONIC_OXYGEN_RATE) / FIRE_DURATION / FIRE_PER_CYCLE;
...
pub const HYDROPONIC_UP_TIME: f32 = EVENT_LOOP_DURATION - (EVENT_DURATION + EVENT_DISTANCE) * (HYDROPONIC_HARVEST_PER_CYCLE + HYDROPONIC_REFIL_PER_CYCLE);
pub const HYDROPONIC_TIME_TO_FOOD: f32 = EVENT_LOOP_DURATION;
pub const HYDROPONIC_WATER_MAX: f32 = 10.0;
pub const HYDROPONIC_WATER_RATE: f32 = HYDROPONIC_WATER_MAX / EVENT_LOOP_DURATION / HYDROPONIC_REFIL_PER_CYCLE;
pub const HYDROPONIC_FOOD_PER_HARVEST: f32 = RACION_SIZE * EATS_PER_LOOP;
pub const WATER_TO_FOOD_RATIO: f32 = (HYDROPONIC_WATER_RATE * HYDROPONIC_TIME_TO_FOOD * 0.5) / HYDROPONIC_FOOD_PER_HARVEST;
...
/// Toilet
pub const PEE_RATE: f32 = (WATER_TO_FOOD_RATIO * RACION_SIZE * EATS_PER_LOOP + DRINK_AMOUNT * DRINK_PER_LOOP) / EVENT_LOOP_DURATION;
