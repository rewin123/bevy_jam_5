...
impl Command for SpawnEarth {
    fn apply(self, world: &mut World) {
        let scene = world.resource::<AssetServer>().load("models/earth.glb#Scene0");
        ...
        let bundle = SceneBundle {
            scene,
            transform: Transform::from_translation(pos).with_scale(Vec3::splat(earth_r)).with_rotation(Quat::from_rotation_x(std::f32::consts::FRAC_PI_2)),
            ..default()
        };
